...
void server(void)
{
  ...
  while(tlist)
  {
    ...
    if(checkall
        || (memicmp(tlist->alist[carea].areaname, bcfg.netserv, namelen)
        == 0))
    {
        ...
        if(taddr)
        {
          ...
          if(tmname)
          {
            ...
            if((blink->echopass[0])
                &&
                (strnicmp
                 (blink->echopass, bufmess.subj,
                  strlen(blink->echopass)) == 0))
            {
              badlink = 0;
            ...
            unlink(tfile);
              ...
              {
            if((msg =
                (short)sopen(tfile, O_RDWR | O_BINARY,
                     SH_DENYWR)) == -1)
            {
            ...
            rread(msg, &bufmsg,
                  (unsigned short)(szmessage - szchar), __FILE__,
                  __LINE__);
            bufmsg.flags |= 4;
...
void doserv(void)
{
  ...
  struct uplname *areas = NULL,
   *tareas = NULL;
  struct uplname ttaname,
   *tname = NULL,
   *tuname = NULL;
  struct alists *ttlist = NULL;
  struct link *blink = NULL,
  ...
  while(!endinput[0])
  {
        ...
        while(ttlist)
        {
          for(i = 0; i < ttlist->numlists; i++)
          {
            if(ttlist->alist[i].type == 1)
            {
              if(bcfg.
              gric
              ? (strichr(blink->group, ttlist->alist[i].group) !=
                 NULL) : (strchr(blink->group,
                         ttlist->alist[i].group) != NULL))
              {
      ...
      if(!endstring[0])
      {
        ...
        else if(strnicmp(token, commands[16], 6) == 0)
        {
          ...
          gric ? (strichr(blink->group, *token) !=
              NULL) : (strchr(blink->group, *token) != NULL))
          {
        ttlist = rlist;
        while(ttlist)
        {
          for(i = 0; i < ttlist->numlists; i++)
          {
            ...
            addsarea(&areas, &ttaname, ttlist->alist[i].areaname,
                 (j == 7) ? 3 : 4, 1);
            }
          }
          ttlist = ttlist->next;
    ...
    sall:
      {
        ...
        {
          if(bcfg.
              gric ? (strichr(blink->group, ttlist->alist[i].group) !=
                  NULL) : (strchr(blink->group,
                          ttlist->alist[i].group) !=
                       NULL))
          {
    ...
    case 18:
      ...
      for(k = 0; k < toklen; k++)
      {
        if(bcfg.
        gric ? (strichr(blink->group, token[k]) !=
            NULL) : (strchr(blink->group, token[k]) != NULL))
        {
          ttlist = rlist;
          while(ttlist)
          {
    ...
    availtoo:
      ...
      if(bcfg.uplname)
      {
        ...
        {
          tupl =
        (short)sopen(tname->echolist, O_RDONLY | O_BINARY, SH_DENYNO);
          if(tupl != -1)
          {
        sprintf(logout, "\r\nUpLink %u:%u/%u.%u\r\n",
        ...
        mywrite(tempsrv, logout, __FILE__, __LINE__);
        curr =
          (unsigned short)rread(tupl, logout, BufSize, __FILE__,
                    __LINE__);
        while(curr)
        {
            ...
            && (bcfg.
                ...
                ? (strichr(blink->group, ttlist->alist[i].group)
                   != NULL) : (strchr(blink->group,
                       ...
                       NULL)))
            {
              addsarea(&areas, &ttaname, ttlist->alist[i].areaname, 1,
                   1);
            }
          }
        }
...
// /*
      if(rulesent
      && (tareas->where == 1 || (tareas->where == 4 && tareas->touched)))
      {
    tlist = rlist;
    while(tlist)
    {
      for(carea = 0; carea < tlist->numlists; carea++)
      {
        ...
        {
          while((rs =
             (unsigned short)rread(newarea->sqd.sqr, logout,
                           BufSize, __FILE__,
                           __LINE__)) > 0)
    ...
    while(tname)
    {
      if(!tname->touch)
      {
        if(cmpaddr(&(tuname->upaddr), &(tname->upaddr)) == 0)
        {
          ...
          sprintf(logout, "%s%s\r", (tname->where == 1) ? "+" : "-",
              tname->persarea);
          mywrite(tempsrv, logout, __FILE__, __LINE__);
          isforw = 1;
          tname->touch = 1;
  ...
  while(ttlist)
  {
    ...
    if(bcfg.
        gric ? (strichr(bcfg.hides, newarea->group) ==
            NULL) : (strchr(bcfg.hides, newarea->group) == NULL))
    {
      ...
      if(bcfg.
          gric ? (strichr(blink->group, newarea->group) ==
              NULL) : (strchr(blink->group, newarea->group) == NULL))
        status = 2;
      else
      {
      ...
      if(!(status == 2 && hide))
      {
        ...
        {
          if(bcfg.gric ? toupper(newarea->group) ==
              toupper(tgroup->group) : (newarea->group ==
                        tgroup->group))
          {
            sprintf(glogout, "\r\n%s\r\n\r\n", tgroup->name);
//                    mywrite(tempsrv,logout,__FILE__,__LINE__);
      ...
      groupwritten:
        ...
        if(ttlist->alist[j].desclen)
        {
          ...
          descr =
        (char *)myalloc((ttlist->alist[j].desclen + 1), __FILE__,
                __LINE__);
          rread(areadesc, descr,
            (unsigned short)(ttlist->alist[j].desclen + 1), __FILE__,
            __LINE__);
        }
        else
        {
          descr =
        (char *)myalloc((strlen(bcfg.mandesc) + 1), __FILE__,
                __LINE__);
        ...
        sprintf(logout, "%s %-10s [%c] %s\r", format, sstatus[status],
            newarea->group, descr);
        mywrite(tempsrv, logout, __FILE__, __LINE__);
          }
          else
        ...
        while(!dend)
        {
          if(!numstr)
          {
            ...
            sprintf(logout, "%s %-10s [%c] ", format, sstatus[status],
                newarea->group);
            numstr = 1;
          }
          else
          ...
          {
            mystrncat(logout, tdesc, (short)(strlen(tdesc) + 1),
                  BufSize);
          ...
          else
          {
            ...
            if(tdesc2 == tdesc)
            {
              mystrncat(logout, tdesc,
                (short)(bcfg.manlen - maxtaglen - 15),
                BufSize);
              tdesc += bcfg.manlen - maxtaglen - 16;
            }
            else
            {
              mystrncat(logout, tdesc, (short)(tdesc2 - tdesc + 1),
                BufSize);
              tdesc = tdesc2;
            }
            mystrncat(logout, "\r", 2, BufSize);
        ...
        if(!bcfg.manlen)
        {
          switch (status)
          {
            case 0:
            case 1:
              sprintf(logout, "%s [%c] %s\r", format, newarea->group,
                  descr);
              break;
            case 3:
              sprintf(logout, "%s (%c) %s\r", format, newarea->group,
...
void makelinks(char *areaname)
{
  ...
  while(ttlist)
  {
    ...
    lseek(areaset, ttlist->alist[j].areaoffs, SEEK_SET);
    rread(areaset, newarea, szarea, __FILE__, __LINE__);
    sprintf(logout, "Area %s on %u:%u/%u.%u is subscribed to:\r\n\r\n",
    ...
    for(i = 0; i < newarea->numlinks; i++)
    {
      ...
      if(!tsnd.deny)
      {
        sprintf(logout, "%u:%u/%u.%u%s%s\r\n", tsnd.zone, tsnd.net,
            tsnd.node, tsnd.point, tsnd.rdonly ? " (Read-only)" : "",
  ...
  while(!endinput[0])
  {
    ...
    nfound:
      ...
      if(tlist->alist[i].desclen)
      {
        ...
        descr =
          (char *)myalloc((tlist->alist[i].desclen + 1), __FILE__,
                  __LINE__);
        rread(areadesc, descr,
          (unsigned short)(tlist->alist[i].desclen + 1), __FILE__,
          __LINE__);
      }
      else
        descr = NULL;
...
//          deladdr(&(newarea->links));
      ...
      else
      {
        ...
        addaddr(&newarea->links, &mfnode);
          ...
          || !(newarea->passthr && (newarea->links.numelem == 1))
          || !bcfg.killorph)
          {
        temp = strstr(string, "-$");
        if(temp)
        {
          wwrite(prttemp, string, (unsigned short)(temp - string + 2),
             __FILE__, __LINE__);
    ...
    delorphb:
      if(bcfg.killorph)
      {
        ...
        while(tlist)
        {
          for(i = 0; i < tlist->numlists; i++)
          {
        if((strnicmp(token, tlist->alist[i].areaname, toklen) == 0)
            && toklen == strlen(tlist->alist[i].areaname))
          goto nfound3;
          }
          tlist = tlist->next;
        }
      nfound3:
         ...
         */
        if(newarea->passthr && (newarea->numlinks <= 1) && bcfg.uplname
        && !blink->noforward)
          delorphn(&utarea, ttname);
        else
          wwrite(prttemp, string, maxstr2[0], __FILE__, __LINE__);
  ...
  while(!endinput[0])
  {
        ...
        && (blink->written == 0))
    {
      ...
      if(blink->mask & 256)
      {
        ...
        mystrncat(logout, tstr, (unsigned short)(strlen(tstr) + 1),
              BufSize);
      }
      if(blink->mask & 1)
      {
      ...
      if(blink->mask & 1024)
        mystrncat(logout, (blink->create == 1) ? "+A " : "-A ", 5,
              BufSize);
      if(blink->mask & 2048)
      {
        switch (blink->boxes)
      ...
      if(blink->mask & 4096)
        mystrncat(logout, (blink->addarc == 1) ? "+C " : "-C ", 5,
              BufSize);
      if(blink->mask & 8192)
        mystrncat(logout, (blink->hold == 1) ? "+H " : "-H ", 5, BufSize);
      if(blink->mask & 16384)
        if(blink->fake)
      ...
      if(blink->mask & 65536L)
        mystrncat(logout, (blink->hide == 1) ? "+R " : "-R ", 5, BufSize);
...
void makemsg(short handle, char *what, char *templ)
{
  ...
  if(!noreport)
  {
    ...
    i++;
      ...
      if((tempsplt =
      (short)sopen("partsplt.$$$", O_RDWR | O_BINARY | O_CREAT, SH_DENYWR,
               S_IRWXU | S_IRWXG | S_IRWXO)) == -1)
      {
    ...
    {
      sprintf(logout, "Splitted by Parma Tosser, part %u from %u\r\r",
          (unsigned short)j, (unsigned short)i);
      mywrite(tempsplt, logout, __FILE__, __LINE__);
/* Maybe temporary included strings */
      sprintf(logout, "%s, part %u from %u", bcfg.subj, (unsigned short)j,
          (unsigned short)i);
...
/* Maybe temporary included strings */
      ...
      while(splitsize < (bcfg.mansize - 100))
      {
        fmax =
          (unsigned short)rread(mhandle, pktbuf,
                    (unsigned
                     short)((bcfg.mansize - splitsize) >
                        bufsize ? bufsize : (bcfg.
                                 mansize -
                                 splitsize)),
                    __FILE__, __LINE__);
        if(fmax)
    ...
    if(strcmp(what, "forwarding"))
      mystrncpy(bufpkt.subj, bcfg.subj, 71);
    sprintf(logout, "Created splitted reply (%s), part %u from %u", what,
        (unsigned short)j, (unsigned short)i);
    logwrite(1, 6);
    if(!quiet)
      ccprintf("%s\r\n", logout);
  ...
  if(sort)
  {
    ...
    for(j = 0; j < ttlist->numlists - 1; j++)
    {
      if(sort == 2)
      {
        ...
        (short)(toupper(ttlist->alist[j].group) -
          ...
          comp =
        (short)(ttlist->alist[j].group - ttlist->alist[j + 1].group);
      }
      else
        comp = 0;
      if(!comp)
    ...
    if(ttlist->next)
    {
      if(sort == 2)
      {
        if(bcfg.gric)
          comp =
        (short)(toupper(ttlist->alist[ttlist->numlists - 1].group) -
      ...
      if(!comp)
        comp =
          (short)stricmp(ttlist->alist[ttlist->numlists - 1].areaname,
          ...
          short where, short wild)
{
  ...
  {
    *chain =
      (struct uplname *)myalloc(sizeof(struct uplname), __FILE__, __LINE__);
    tareas = *chain;
  }
  else
  {
