...
void readsetup (void)
{
  ...
  char *temp = NULL, firstnet[arealength], curpath[(DirSize + 1)];
  struct myaddr *taddr = NULL;
  struct uplname *tupl = NULL, *ttupl = NULL;
	  ...
	  if (tlist->alist[i].type == 0)
	      ...
	      if (!outset)
		{
		  if ((strlen (bcfg.netout) == strlen (newarea->areaname)) &&
		      (strnicmp
		       (bcfg.netout, newarea->areaname,
			strlen (bcfg.netout)) == 0))
		    {
		      mystrncpy (outpath, newarea->areafp, DirSize);
		      outset = 1;
		    }
	      ...
	      if (!servset)
		{
		  if ((strlen (bcfg.netserv) == strlen (newarea->areaname)) &&
		      (strnicmp
		       (bcfg.netserv, newarea->areaname,
			strlen (bcfg.netserv)) == 0))
		    {
  ...
  if (bcfg.logfile[0])
    {
      ...
      if (temp)
	{
	  memcpy (logout, bcfg.logfile, (unsigned)(temp - bcfg.logfile + 1));
	  createpath (logout);
	}
    }
  if (bcfg.semaphore)
      ...
      while (tupl)
	{
	  ...
	  if (temp)
	    {
	      memcpy (logout, tupl->echolist,
		      (unsigned)(temp - tupl->echolist + 1));
	      createpath (logout);
	    }
	  tupl = tupl->next;
	}
