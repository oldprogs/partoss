...
void linksqd(void)
{
  ...
  if(maxmsg)
  {
	...
	if(msgid[(unsigned)i])
	{
	  for(j = 0; j < maxmsg; j++)
	  {
	    if((reply[(unsigned)j] == msgid[(unsigned)i]) && (i != j))
	      setreply(i + 1, j + 1);
	  }
	}
      }
  ...
  if(linktype != 2)
  {
	...
	temp2++;
      ...
      memcpy(tmsgid, temp + 7,
	     (unsigned)(((temp2 - temp - 7) > 79) ? 79 : (temp2 - temp - 7)));
      tmsgid = strupr(tmsgid);
      msgid[(unsigned)number] = crc32block(tmsgid, (short)strlen(tmsgid));
    }
    else
	...
	temp2++;
      ...
      tmsgid = strupr(tmsgid);
      reply[(unsigned)number] = crc32block(tmsgid, (short)strlen(tmsgid));
    }
    else
