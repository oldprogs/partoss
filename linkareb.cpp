...
void dbg_free(void **ptr, const char *file_name, unsigned file_line)
{
  ...
  if(ptr == NULL)
  {
    fprintf(stderr, "Can't free NULL pointer to pointer at line %u of %s\n",
	    file_line, file_name);
    exit(1);
  }
  if(*ptr == NULL)
...
typedef struct MsgInfo {
  ...
  MsgInfo *next,
   *prev;			// Указатели на следующее и предыдущее письма в
  // цепочке.
...
unsigned long str2MSGID(const char *str)
{
  unsigned long idval = 0,
    m;
  int i;
  const char *p = (str[0] == '\001' ? str + 1 : str);
  char *p1 = strchr(p, '\001'),
   *p2 = strchr(p, '\000');
...
void markList(PHash umsghash, PMsgInfo msg)
{
  ...
  while(curMsg && (curMsg->nextreply[0] != 0))
  {
	...
	if((curMsg->next->prev != curMsg) || (curMsg->next->next == curMsg)
	    || (curMsg->next == curMsg))
	{
	  curMsg->nextreply[0] = 0;
	  curMsg->mark_changed();
		...
		unsigned long frames, int linkType)
{
  ...
  if(linkType == LINKTYPE_REPLY)
  {
    ...
    fakemsgs =
      (PMsgInfo *) myalloc(sizeof(PMsgInfo) * fakesize, __FILE__, __LINE__);
  }
  ...
  for(i = 0; i < frames; i++)
  {
	...
	fakesize += fakechunk;
	fakemsgs =
	  (PMsgInfo *) myrealloc(fakemsgs, sizeof(PMsgInfo) * fakesize,
				 __FILE__, __LINE__);
      }
      fakemsgs[fakecount] =
...
int linkArea(const char *areaFile, int linkType)
{
  ...
  long i;
  int hi,
    hd;				// Хендлы для файловых операций.

  // hi - для индекса, hd - для базы.
  char index_file[_MAX_PATH],
    base_file[_MAX_PATH];
  ...
  PHash msghash,
    umsghash;			// Хэши с индексированием по MSGID и umsgid соотв.
  ...
  for(i = 0; i < frames; i++)
  {
	...
	&&
	(memcmp(msgs[i].nextreply + 1, zero_array, 9 * sizeof(unsigned long))
	 != 0))
    {
