  ...
  while(!endinput[0])
  {
	...
	if(memicmp(token, "Route", 5) == 0)
	{
	      ...
	      || token[0] == '.')
	  {
	    if(roulist == NULL)
	    {
	      roulist = (struct route *)myalloc(szroute, __FILE__, __LINE__);
	      trou = roulist;
	    }
	    else
	    {
		...
		trou = trou->next;
	      trou->next =
		(struct route *)myalloc(szroute, __FILE__, __LINE__);
	      trou = trou->next;
	    }
	...
	else
	{
	  if(memicmp(token, "Direct", 6) == 0)
	  {
	    ...
	    if(direct == NULL)
	    {
	      direct = (struct route *)myalloc(szroute, __FILE__, __LINE__);
