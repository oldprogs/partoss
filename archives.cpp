...
*/
{
  ...
  if(type == 2 && arclen <= 0)
    return -1;
  /* Парсим конфигурацию архиваторов */
  numofarc = (short)(filelength(compset) / szpacker);
  lseek(compset, 0, SEEK_SET);
  if(type == 1) // Если распаковка - сразу сверяем сигнатуры, чтоб найти подходящий распаковщик
  {
  ...
  if(i < numofarc) // Готовимся к запуску архиватора
  {
    j = 0;
    ...
    while(temp && *temp) // Разбор и подстановка ключей %A %F %P
    {
      temp2 = temp;
      ...
      if((*temp2) == '%')
      {
        ...
        {
          mystrncat(param[j], (char *)(type == 1 ? fulname : arcname),
            DirSize, DirSize);
          temp2 += 2;
        }
        else
...
// #endif
    ...
    if(type == 1)
      ...
      sprintf(logout, "%sing %lu bytes for %u:%u/%u.%u", arcdef.name, arclen,
          tpack->outaddr.zone, tpack->outaddr.net, tpack->outaddr.node,
          tpack->outaddr.point);
    logwrite(1, 2);
...
#endif /* __LNX__ */
#endif /* __NT__ */
    mystrncpy(packf, param[numf], DirSize); // ??? Похоже на упаковку списка... Нифига не понял.
    temp = strrchr(packf, '.');
    if(temp)
      temp[1] = 0;
    mystrncat(packf, "ouf", 6, DirSize);
    ...
    else
    {
      ...
      if(bcfg.renbad || (type != 1))
      {
        ...
        if(rrename((type == 1 ? fulname : packname), badname))
          sprintf(logout, "??? Rename of file %s (Archiver's error %X) fails",
