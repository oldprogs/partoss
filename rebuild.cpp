  ...
  if(needwork)
  {
	...
	for(i = 0; i < maxmsg; i++)
	{
	  ...
	  if(diff >= newarea->days)
	  {
	    ...
	    {
	      bitmap[(unsigned)(i >> 3)] &= (char)((0xfe << (i & 7)) & 0xff);
	      if(bcfg.analyse == 3)
		sumdel++;
	    }
	  }
	  else if(bcfg.analyse > 1)
	    bitmap[(unsigned)(i >> 3)] |= (char)((1 << (i & 7)) & 0xff);
	mnext:
	  ;
	}
      }
...
void untoss(void)
{
  ...
  for(i = 0; i < maxmsg; i++)
  {
	...
	{
	  wwrite(bcurr, sqdbuf, (unsigned short)(temp - sqdbuf + 1), __FILE__,
		 __LINE__);
	  break;
	}
	else
	...
	wwrite(bcurr, bufsqd.text, (unsigned short)(temp - bufsqd.text + 1),
	       __FILE__, __LINE__);
      else
