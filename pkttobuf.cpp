...
//  pcurpos+=34;
  ...
  if(temp)
  {
	...
	pcmsglen = j;
      if((mode & 2 || rescan || bcfg.lookpath) && (newarea->numlinks > 1))
	parsesnb(temp, 1);
    }
    else
    {
      ...
      sprintf(logout,
	      ...
	      bufpkt.fromnode, bufpkt.frompoint, bufpkt.toname, bufpkt.tozone,
	      bufpkt.tonet, bufpkt.tonode, bufpkt.topoint, bufpkt.subj);
      logwrite(1, 3);
      if(!quiet)
	ccprintf("%s\r\n", logout);
    }
  ...
  else
  {
    ...
    wwrite(temppkt, bufpkt.toname, (unsigned short)(ptolen + 1), __FILE__,
	   __LINE__);
    wwrite(temppkt, bufpkt.fromname, (unsigned short)(pfromlen + 1), __FILE__,
	   __LINE__);
    ...
    wwrite(temppkt, pktbuf + pcurpos + pkludlen,
	   (unsigned short)(fmax - pcurpos - pkludlen), __FILE__, __LINE__);
    while(!endmsg)
    {
      fmax2 = (unsigned short)rread(pkt, pktbuf, buflen, __FILE__, __LINE__);
