	      ...
	      unsigned short node)
{
  ...
  {
    tsnb = chain->last;
    tsnb->next = (struct shortaddr *)myalloc(szshortaddr, __FILE__, __LINE__);
    tsnb = tsnb->next;
  }
  chain->last = tsnb;
  tsnb->next = NULL;
...
void addname(struct namechain *chain, struct sysname *name)
{
  ...
  if(chain->numelem == 0)
  {
    chain->chain = (struct sysname *)myalloc(szsysname, __FILE__, __LINE__);
    tname = chain->chain;
  }
  else
  {
