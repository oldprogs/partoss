...
void gettoken(short level)
{
  ...
  if(string[curtpos[level]] == '"')
    curtpos[level]++;
  if(curtpos[level] >= maxstr[level] || string[j] == '\r' || string[j] == '\n'
      || string[j] == 0x1a)
    endstring[level] = 1;
  toklen = (short)(j - i);
}
...
#if HAVE_SOPEN
    if((logfile = (short)sopen(hfile, O_RDWR | O_BINARY, SH_DENYNO)) == -1)
      if((logfile =
...
void badlog(struct area *barea)
{
  ...
  if(bcfg.badlog[0])
  {
    ...
    case 'P':
      ...
      break;
    case 'A':
      sprintf(logout, "%u:%u/%u.%u", barea->myaka.zone, barea->myaka.net,
          barea->myaka.node, barea->myaka.point);
...
//    mystrncat(tstr,"\r\n",5,CSSize);
    ...
    if((blog = (short)sopen(bcfg.badlog, O_RDWR | O_BINARY, SH_DENYWR)) == -1)
    {
      if((blog =
      (short)sopen(bcfg.badlog, O_RDWR | O_BINARY | O_CREAT, SH_DENYWR,
               S_IRWXU | S_IRWXG | S_IRWXO)) == -1)
      {
...
void addarcs(char *path, struct find_t *fblk, short secure)
{
  ...
  else
  {
    ...
    if(cpkt == NULL)
    {
      ...
      while(cpkt->next)
    cpkt = cpkt->next;
      cpkt->next = (struct pktname *)myalloc(szpktname, __FILE__, __LINE__);
      cpkt = cpkt->next;
    }
    else
      return;
...
short rrename(char *from, char *to)
{
  ...
  if(memicmp(from, to, len))
  {
    ...
    logwrite(1, 1);
    sprintf(logout, "Previous try returned %d, errno - %d", result, terr);
    logwrite(1, 1);
      }
      mtsleep(1);
    }
...
short movefile(char *src, char *dst)
{
  ...
  while(res > 0)
  {
    wwrite((short)dsth, sbuffer[0], (unsigned short)res, __FILE__, __LINE__);
    res = rread((short)srch, sbuffer[0], 4096, __FILE__, __LINE__);
  }
  Close(dsth);
  Close(srch);
