         ...
         short pers)
{
  ...
  if(ttarea->type)
  {
    ...
    switch (terrno)
    {
      case ENOENT:
        ...
        else
        {
          ...
          mystrncat(sqbuf.base, ttarea->areafp + (fplen - 72), 79, 79);
        }
        sqbuf.szsqhdr = 28;
        if((ttarea->sqd.sqd =
        (short)sopen(sqdfile, O_RDWR | O_BINARY | O_CREAT, shdeny,
    ...
    ttnum =
      (unsigned short)(((sqbuf.nummsg < bufsqi) ? sqbuf.nummsg : bufsqi) *
               12);
    tnummsg =
      (unsigned short)rread(ttarea->sqd.sqi, tindex, ttnum, __FILE__,
                __LINE__);
    if(tnummsg == 65535u)
...
//          sqbuf.nummsg=sqbuf.highmsg=(tnummsg/12);
  ...
  passthru:
    ...
    else
    {
      ...
      rread(ttarea->sqd.sqb, dupbuf, (ttarea->dupes * szlong), __FILE__,
        __LINE__);
    }
      }
    }
...
void closesqd(struct area *ttarea, short dup)
{
  ...
  if(ttarea->type)
  {
    ...
    if(ttarea->duptouch)
    {
      ...
      wwrite(ttarea->sqd.sqb, dupbuf, (ttarea->dupes * szlong), __FILE__,
         __LINE__);
    }
...
void createarea(char *areaname, short pers, struct myaddr *pktaddr2)
{
  ...
  unsigned short autonum = 0,
    tnum,
    fplen;
  unsigned short date,
    time /*,fyear,fmonth,fday */ ;

  // unsigned short fhour,fmin,fsec;
  char newname[BufSize + 1],
    tareaname[arealength + 1],
    autotemp[5],
   *temp = NULL,
   *temp2 = NULL;
  struct areaindex *anew = NULL;
  struct area *crarea = NULL,
    tdefarea;
  ...
  if(bladv)
  {
    ...
    for(i = 0; i < tlist->numlists; i++)
    {
      if(stricmp(tlist->alist[i].areaname, bladv->persarea) == 0
          && strlen(tlist->alist[i].areaname) == strlen(bladv->persarea))
      {
        lseek(areaset, tlist->alist[i].areaoffs, SEEK_SET);
...
founddef:
  ...
  else
  {
    ...
    (unsigned char *)myalloc(8192, __FILE__, __LINE__);
  ...
  while(taddr)
  {
    if(taddr->point || ((pers >= 2) ? mfnode.point : pktaddr.point))
       ...
       */
    {
      if((taddr->zone == pktaddr.zone) && (taddr->net == pktaddr.net)
      && (taddr->node == pktaddr.node))
    break;
    }
...
*/
  ...
  {
    tlist->next =
      (struct alists *)myalloc((sizeof(struct alists) << bcfg.grof), __FILE__,
                   __LINE__);
    tlist = tlist->next;
    tlist->alist =
  ...
  if(pers != 2)
  {
    ...
    if(setf == -1)
      setf =
    (short)sopen(bcfg.creatfile, O_RDWR | O_BINARY | O_CREAT, SH_DENYWR,
...
goodnam2:
  ...
  sprintf(logout, " -p%u:%u/%u.%u", ttarea->myaka.zone, ttarea->myaka.net,
      ttarea->myaka.node, ttarea->myaka.point);
  ...
  while(link)
  {
    ...
    if(tsnd.zone != link->zone)
      sprintf(logout, " %s%s%s%u:%u/%u.%u",
          ...
          link->rdonly ? "x" : "", link->passive ? "y" : "", link->zone,
      ...
      sprintf(logout, " %s%s%s%u.%u",
          (link->rdonly | link->passive) ? "-" : "",
          link->rdonly ? "x" : "", link->passive ? "y" : "", link->node,
          link->point);
    else
    {
      sprintf(logout, " %s%s%s.%u", (link->rdonly | link->passive) ? "-" : "",
          link->rdonly ? "x" : "", link->passive ? "y" : "", link->point);
      i = 0;
    }
    if(link->point == 0 && i)
    {
