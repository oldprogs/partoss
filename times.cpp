...
{
  short i;
  unsigned short ayear, amonth, aday, ahour, amin, atsec;
  unsigned short byear, bmonth, bday, bhour, bmin, btsec;
...
fixt:
  if (i)
    if (ahour > bhour || (ahour == bhour && amin > bmin) || (ahour == bhour &&
							     amin == bmin
							     && atsec >
							     btsec))
      i--;
...
short converttime (char *time)
{
  ...
  result = (ttmt.tm_mon > 0 &&
	    ...
	    ttmt.tm_hour < 24 &&
	    ttmt.tm_min >= 0 &&
	    ttmt.tm_min < 60 && ttmt.tm_sec <= 0 && ttmt.tm_sec < 31);
  if (ttmt.tm_year < 80)
    ttmt.tm_year += 80;
  if (ttmt.tm_mday < 10)
    ftstime[0] = '0';
