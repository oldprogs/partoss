...
void runmainset(void)
{
  ...
  while(!finish)
  {
    ...
    case ENOENT:
      if((areaset =
          (short)sopen(cfname, O_RDWR | O_BINARY | O_CREAT, SH_DENYWR,
...
//    printf("umask: %d & 0777 = %d\n",umask_val,(umask_val&777));
      ...
      case 0:
        ...
        bcfg.numaddr++;
          if(!(defaddr.zone || defaddr.net || defaddr.node
           || defaddr.point))
...
//По идее, тут надо бы сделать mappath в случае необходимости.
        ...
        case '-':
          switch (token[1])
          {
            ...
            case 'N':
              ...
              {
                descr =
                  (char *)myalloc((toklen + 1), __FILE__,
                          __LINE__);
            ...
            case 'F':
              ...
              if(i < 21)
              {
                ...
                if(pool[strlen(pool) - 1] != DIRSEP[0])
                  mystrncat(pool, (char *)DIRSEP, 3, DirSize);
              }
              else
              {
                ...
                if(bcfg.areapool[strlen(bcfg.areapool) - 1] !=
                DIRSEP[0])
        ...
        (ispool ? (lseek(areapool, 0, SEEK_END)) : 0);
          ...
          if(isdescr)
          {
        wwrite(areadesc, descr, (unsigned short)(aindex.desclen + 1),
               __FILE__, __LINE__);
//                myfree((void **)&descr,__FILE__,__LINE__);
          }
          if(ispool)
          {
        wwrite(areapool, pool, (unsigned short)(ttarea1->poollen + 1),
               __FILE__, __LINE__);
//                myfree((void **)&pool,__FILE__,__LINE__);
        ...
        while(temp && *temp)
        {
          while(*temp2 && (*temp2 != '@'))
            temp2++;
          wwrite(edlist, temp, (unsigned short)(temp2 - temp),
             __FILE__, __LINE__);
          temp2++;
          switch (toupper(*temp2))
          {
            ...
            mywrite(edlist, ttarea1->areaname, __FILE__,
                __LINE__);
              mywrite(edlist, "\"", __FILE__, __LINE__);
              break;
            case 'A':
              sprintf(logout, "%u:%u/%u.%u", ttarea1->myaka.zone,
                  ttarea1->myaka.net, ttarea1->myaka.node,
                  ttarea1->myaka.point);
              mywrite(edlist, logout, __FILE__, __LINE__);
              break;
            default:
...
*/
      ...
      case 89:
        ...
        {
          bcfg.dgate =
        (struct dngate *)myalloc(sizeof(struct dngate), __FILE__,
                     __LINE__);
          tdn = bcfg.dgate;
        }
        else
          ...
          else
          {
            ...
            if(strnicmp(token + 1, "%All", 4) == 0)
              ...
              mystrncpy(tdn->where, token + 1,
                (short)((toklen >
                     arealength ? arealength : toklen) -
                    1));
          }
          break;
        case '$':
          mystrncpy(tdn->origin, token + 2,
                (short)((toklen > 72 ? 72 : toklen) - 2));
          break;
        case '%':
          for(i = 1; i < toklen; i++)
          {
