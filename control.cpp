...
void writechain(short handle, struct shortchain *chain, short type)
{
  ...
  for(i = 0; i < chain->numelem; i++)
  {
    if(!tsnb->hide)
    {
      mystrncpy(curnode, (char *)" ", 2);
      if(tsnb->net != net)
      {
	mystrncat(curnode, (char *)itoa(tsnb->net, temp, 10), 8, 12);
	mystrncat(curnode, (char *)"/", 2, 12);
      }
      mystrncat(curnode, (char *)itoa(tsnb->node, temp, 10), 8, 12);
...
void parsesnb(char *temp, short type)
{
  ...
  if(type == 1 && !(newarea->myaka.point) && bcfg.lookpath)
  {
    ...
    {
      if(tsnb->net == newarea->myaka.net && tsnb->node == newarea->myaka.node
	  && tsnb->next)
      {
	isdupe = 2;
	//
	...
	if(bcfg.loglevel > 10)
	{
	  sprintf(logout,
		  ...
		  newarea->myaka.node, newarea->myaka.point, tsnb->net,
		  tsnb->node);
	  logwrite(1, 11);
	}
	//
  ...
  while(link)
  {
	...
	if(link->point == 0)
	{
	  ...
	  for(i = 0; i < (type == 1 ? psnb.numelem : ssnb.numelem); i++)
	  {
	...
	if(present)
	{
	  ...
	  for(i = 0; i < (type == 1 ? ppath.numelem : spath.numelem); i++)
	  {
	    if(tsnb->net == link->net && tsnb->node == link->node
	...
	if(type == 1)
	{
	  if(pktaddr.zone == link->zone && pktaddr.net == link->net
	      && pktaddr.node == link->node && pktaddr.point == link->point)
	    present = 0;
	}
	else
	{
	  if(bufsqd.fromzone == link->zone && bufsqd.fromnet == link->net
	...
	if(!bcfg.selfaka)
	{
	  ...
	  while(taddr)
	  {
		...
		&& taddr->node == link->node && taddr->point == link->point)
	      present = 0;
	    taddr = taddr->next;
	  }
	}
...
void sortsnb(struct shortchain *chain)
{
  ...
  {
    tmp =
      (unsigned long *)myalloc((chain->numelem * szlong), __FILE__, __LINE__);
    tsnb = chain->chain;
    for(i = 0; i < chain->numelem; i++)
    {
      tmp[i] = tsnb->net;
...
//    tkludge->str[i]=0;
    ...
    if(local && 1)
    {
      tkludge->next = (struct kludge *)myalloc(szkludge, __FILE__, __LINE__);
      tkludge = tkludge->next;
      tkludge->next = NULL;
      memset(att, 0, DirSize + 1);
      sprintf(att, "\1TID: ParToss %s", version);
      tkludge->str = (char *)myalloc(strlen(att) + 3, __FILE__, __LINE__);
...
//              while(!isspace(*temp4))
		temp4--;
	      ...
	      memcpy(curmsgid, temp4,
		     (unsigned)(((temp2 - temp4) > 8) ? 8 : (temp2 - temp4)));
	    }
	  }
	  break;
	case 2:
...
//          tbufmess->frompoint=faddr.point;
	...
	case 7:
	  if(mode & 2048)
	  {
	    ...
	    if(!isarea)
	    {
	      ...
	      if(arealen > arealength + 4)
		arealen = arealength + 4;
	      memcpy(curarea, temp + 6 + spaces, arealen - 5 - spaces);
	      areaaliasmaker();
	      arealenold = arealen;
	      arealen = strlen(curarea) + 5;
	      setarea(curarea, 0);
	...
	case 8:
	  ...
	  if(temp3 == NULL)
	  {
	    if(newarea->pooloffs)
	    {
	      ...
	      rread(areapool, att, (unsigned short)(newarea->poollen + 1),
		    __FILE__, __LINE__);
	    }
	    else
	      memset(att, 0, DirSize + 1);
	  ...
	  if(glpool)
	  {
	    ...
	    while(tpool->next)
	      tpool = tpool->next;
	    tpool->next =
	      (struct pool *)myalloc(sizeof(struct pool), __FILE__, __LINE__);
	    tpool = tpool->next;
	  }
	  else
	  {
	  ...
	  tpool->type = (char *)myalloc(strlen(tatt) + 1, __FILE__, __LINE__);
	  tpool->next = NULL;
	...
	*ckludge = (struct kludge *)myalloc(szkludge, __FILE__, __LINE__);
	...
	tkludge->str[strlen(logout)] = 0;
      ...
      {
	tkludge->str =
	  (char *)myalloc((unsigned)(temp2 - temp + 3), __FILE__, __LINE__);
