...
short templat(short fhandle, short thandle)
{
  short retfile,
    ...
    thnd;
  unsigned short j;
  char *alias[] = { "tfname", "tlname", "tname", "ffname", "flname", "fname",
    "taddr", "faddr", "date", "time", "text", "subj", "area", "version",
    "return", "file:"
  };
  char *temp = NULL,
   *temp2 = NULL,
  ...
  retfile =
    (short)sopen(pttmpl, O_RDWR | O_BINARY | O_CREAT, SH_DENYWR,
  ...
  while(!endinput[1])
  {
    ...
    if(temp)
    {
      wwrite(retfile, temp2, (unsigned short)(temp - temp2), __FILE__,
         __LINE__);
      ...
      switch (i)
      {
        ...
        wwrite(retfile, bufpkt.toname,
               (unsigned short)(temp3 - bufpkt.toname), __FILE__,
               __LINE__);
          else
        ...
        wwrite(retfile, bufpkt.fromname,
               (unsigned short)(temp3 - bufpkt.fromname), __FILE__,
               __LINE__);
          else
        ...
        case 6:
          sprintf(logout, "%u:%u/%u.%u", bufpkt.tozone, bufpkt.tonet,
              bufpkt.tonode, bufpkt.topoint);
          mywrite(retfile, logout, __FILE__, __LINE__);
          break;

        case 7:
          sprintf(logout, "%u:%u/%u.%u", bufpkt.fromzone, bufpkt.fromnet,
        ...
        while((fmax =
               (unsigned short)rread(fhandle, pktbuf, buflen,
                         __FILE__, __LINE__)) != 0)
        {
          if(bcfg.transl)
          {
          ...
          wwrite(retfile, pktbuf, fmax, __FILE__, __LINE__);
        }
          }
          break;
        ...
        case 12:
          mywrite(retfile, newarea->areaname, __FILE__, __LINE__);
          break;
        ...
        temp2++;
          ...
          if((thnd =
          (short)sopen(logout, O_RDONLY | O_BINARY, SH_DENYWR)) != -1)
          {
        lseek(thnd, 0, SEEK_SET);
        while((fmax =
               (unsigned short)rread(thnd, pktbuf, buflen, __FILE__,
                         __LINE__)) != 0)
        {
          for(j = 0; j < fmax; j++)
          {
      ...
      temp =
        (char *)memchr(temp2, 0x40,
               (unsigned)(maxstr2[1] - (temp2 - string)));
    }
    else
    {
      wwrite(retfile, temp2,
         (unsigned short)(maxstr2[1] - (temp2 - string)), __FILE__,
         __LINE__);
