...
*/
  ...
  while(tkludge)
  {
	...
	if(match && tdn->changes & 2)
	{
	  ...
	  sprintf(logout, "\1MSGID: %u:%u/%u.%u %s\r", tdn->aka.zone,
		  tdn->aka.net, tdn->aka.node, tdn->aka.point, msgid);
  ...
  if(bigmess)
  {
	...
	if(temp)
	{
	  if(tdn->origin[0])
	  {
	    wwrite(tpack->handle, tbuf, (unsigned short)(temp - tbuf + 11),
		   __FILE__, __LINE__);
	  ...
	  sprintf(logout, "(%u:%u/%u.%u)\r", glink->zone, glink->net,
		  glink->node, glink->point);
	  mywrite(tpack->handle, logout, __FILE__, __LINE__);
	}
	else
	...
	{
	  wwrite(tpack->handle, tbuf, (unsigned short)(temp - tbuf), __FILE__,
		 __LINE__);
	  break;
	}
      }
  ...
  else
  {
	...
	if(tdn->origin[0])
	{
	  wwrite(tpack->handle, buftemp->text,
		 (unsigned short)(temp - buftemp->text + 11), __FILE__,
		 __LINE__);
	  mywrite(tpack->handle, tdn->origin, __FILE__, __LINE__);
	}
	else
	{
	  ...
	  while(*temp != '(')
	    temp--;
	  wwrite(tpack->handle, buftemp->text,
		 (unsigned short)(temp - buftemp->text), __FILE__, __LINE__);
	}
	sprintf(logout, " (%u:%u/%u.%u)\r", glink->zone, glink->net,
		glink->node, glink->point);
	mywrite(tpack->handle, logout, __FILE__, __LINE__);
...
*/
    sprintf(logout, "\1Via ParToss %s %u:%u/%u.%u, %s\r", version,
	    tpack->fromaddr.zone, tpack->fromaddr.net, tpack->fromaddr.node,
	    tpack->fromaddr.point, ftstime);
    mywrite(tpack->handle, logout, __FILE__, __LINE__);
//     }
  }
  else
  {
	...
	if(tsend->zone == newarea->myaka.zone)
	{
	  ...
	  writechain(tpack->handle, (type == 1 ? &ppath : &spath), 2);
	}
	else
	{
	  zg = bcfg.zonegate;
