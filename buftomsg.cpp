...
void buftomsg(short type)
{
   ...
   *temp2 = NULL,
   *null1 = "\0";
  char dupbase[DirSize + 1],
    tareaname[arealength + 1];
  short msg = 0,
    inout = 0,
    dupe = 0;
  int finish = 0;
  unsigned short tmsg = 0,
    fmax2 = 0,
    tnummsg;
  unsigned short bfromzone = 0,
    bfrompoint = 0;
  unsigned short btozone = 0,
    btopoint = 0;
  struct find_t fblk;
...
nfound:
  ...
  if(type != 1 && type != 4)
  {
    if(bcfg.netdupes && newarea->dupes)
    {
      ...
      if((dupe = (short)sopen(dupbase, O_RDWR | O_BINARY, SH_DENYWR)) == -1)
      {
	if((dupe =
	    (short)sopen(dupbase, O_RDWR | O_BINARY | O_CREAT, SH_DENYWR,
	...
	wwrite(dupe, dupbuf, (newarea->dupes * szlong), __FILE__, __LINE__);
      }
      else
      {
	rread(dupe, &dupemax, 2, __FILE__, __LINE__);
	rread(dupe, &dupecur, 2, __FILE__, __LINE__);
	rread(dupe, dupbuf, (newarea->dupes * szlong), __FILE__, __LINE__);
      }
      dupcheck(3);
      if(!isdupe)
      {
  ...
  if(mbigmess && type != 4 /* From Server */ )
  {
    ...
    rread(tempmsg, bufmess.toname, (unsigned short)(mtolen + 1), __FILE__,
	  __LINE__);
    rread(tempmsg, bufmess.fromname, (unsigned short)(mfromlen + 1), __FILE__,
	  __LINE__);
  ...
  if(type == 1 || type == 4)
  {
    ...
    msgid[8] = 0;
    sprintf(logout, "\1MSGID: %u:%u/%u.%u %s\r", bfromzone, bufmsg.fromnet,
	    bufmsg.fromnode, bfrompoint, msgid);
    mywrite(msg, logout, __FILE__, __LINE__);
    if(bcfg.fintl || (bfromzone != btozone))
    {
      sprintf(logout, "\1INTL %u:%u/%u %u:%u/%u\r", btozone, bufmsg.tonet,
	      bufmsg.tonode, bfromzone, bufmsg.fromnet, bufmsg.fromnode);
      mywrite(msg, logout, __FILE__, __LINE__);
    }
    if(bfrompoint)
    {
  ...
  {
    while((fmax2 =
	   (unsigned short)rread(tempmsg, type == 4 ? sqdbuf : pktbuf, buflen,
				 __FILE__, __LINE__)) != 0)
    {
      fakeorigin(fake, type == 4 ? sqdbuf : pktbuf, fmax2);
      wwrite(msg, type == 4 ? sqdbuf : pktbuf, fmax2, __FILE__, __LINE__);
    }
  }
  else
  {
