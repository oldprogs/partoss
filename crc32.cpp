...
unsigned long crc32block (char *ptr, short count, unsigned long tcrc)
{
  short i = 0;
  char *temp = NULL;
  temp = ptr;
  for (i = 0; i < count; i++, temp++)
    {
      tcrc =
	cr3tab[((short)tcrc ^ *temp) & 0xff] ^ ((tcrc >> 8) & 0x00FFFFFFul);
    }
  return tcrc;
}
...
void dupcheck (short type)
{
  unsigned short i = 0;
  isdupe = 0;
  if (((type != 2) ? msgcrc32 : smsgcrc32) == 0L)
    {
      //
      // For Jury Fradkin
      //
...
//   {
  for (i = 0; i < dupemax; i++)
    {
      if (dupbuf[i] == ((type != 2) ? msgcrc32 : smsgcrc32))
	{
	  isdupe = 1;
	  //
	  // For Jury Fradkin
...
//   }
  if (!isdupe)
      ...
      if (bcfg.loglevel > 10)
	{
      ...
      // For Jury Fradkin
      //
      if (dupemax < newarea->dupes)
	dupemax++;
      dupecur++;
      if (dupecur > newarea->dupes)
	dupecur = 1;
      dupbuf[dupecur - 1] = ((type != 2) ? msgcrc32 : smsgcrc32);
      newarea->duptouch = 1;
    }
}
