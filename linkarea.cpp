...
unsigned long str2MSGID(const char *str)
{
  ...
  {
    idval =
      idval * 16 + (isalpha(str[i]) ? toupper(str[i]) - 'A' + 10 : str[i] -
		    '0');
  }
  return idval;
}
		...
		unsigned long frames, int linkType)
{
  ...
  if(linkType == LINKTYPE_REPLY)
  {
    ...
    fakemsgs =
      (PMsgInfo *) myalloc(sizeof(MsgInfo) * fakesize, __FILE__, __LINE__);
  }
  ...
  for(i = 0; i < frames; i++)
  {
	...
	fakesize += fakechunk;
	fakemsgs =
	  (PMsgInfo *) myrealloc(fakemsgs, fakesize, __FILE__, __LINE__);
      }
      fakemsgs[fakecount] =
