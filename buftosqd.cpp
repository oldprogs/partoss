...
short buftosqd(struct area *ttarea, struct sqifile *tindex, short type)
{
  ...
  sprintf(logout, "***** Debug info: [%s] [%s] [%s] [%s]", ttarea->areaname,
  ...
  else
  {
    ...
    tkllen = skludlen;
    wipearea = 1;
    kill = (mode & 2176 && flags2 && (ttarea->type == 1 || ttarea->type == 3)
	    && bcfg.ktrash);
  }
// /*
  if(ttarea->type == 2)
  {
	...
	ttkludge = ttkludge->next;
      ttkludge->next = (struct kludge *)myalloc(szkludge, __FILE__, __LINE__);
      ttkludge = ttkludge->next;
    }
    ttkludge->next = NULL;
...
// */
  ...
  if(!ttarea->passthr)
  {
	...
	rread(ttarea->sqd.sqi, tindex, (unsigned short)(bufsqi * 12),
      ...
      memcpy(&sqibuf, &tindex[(unsigned)(sqbuf.skipmsg - ttarea->curindex)],
	     12);
	...
	if(sqbuf.lastf)
	{
	  ...
	  pcurr.next = 0;
	  if(sqread(ttarea, prevf, &pprev, __FILE__, __LINE__) == -1)
	    return -1;
  ...
  while(tkludge)
  {
	...
	if(!isdog)
	{
      ...
      memcpy(klbuf + (unsigned)tislen, tkludge->str, strlen(tkludge->str));
...
//              temp=locseenby(tsbuf);
	...
	{
	  wwrite(ttarea->sqd.sqd, tsbuf, (unsigned short)(temp - tsbuf),
		 __FILE__, __LINE__);
	  reallen += (temp - tsbuf);
	  break;
	}
  ...
  else
  {
	...
	if((bcfg.delinfo && !ttarea->saveci) && kill2 && ttarea->type == 1)
	{
	  ...
	  {
	    wwrite(ttarea->sqd.sqd, buftemp->text, (unsigned short)(textlen),
		   __FILE__, __LINE__);
	    reallen += textlen;
	  }
	}
