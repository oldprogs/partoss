...
//      if(!(mode&2176))
    ...
    else
    {
      ...
      wwrite(tempsqd, bufsqd.toname, (unsigned short)(stolen + 1), __FILE__,
	     __LINE__);
      wwrite(tempsqd, bufsqd.fromname, (unsigned short)(sfromlen + 1),
	     __FILE__, __LINE__);
      wwrite(tempsqd, bufsqd.subj, (unsigned short)(ssubjlen + 1), __FILE__,
	     __LINE__);
      fmax2 =
...
//      getctrl(sqdbuf,(fmax2<head.clen)?(fmax2-1):head.clen,2);
      smsglen = fmax2;
      if(gheadgarb)
	wwrite(tempsqd, sqdbuf + skludlen, (unsigned short)(fmax2 - skludlen),
	       __FILE__, __LINE__);
      else
	wwrite(tempsqd, sqdbuf + skludlen + 1,
	       (unsigned short)(fmax2 - skludlen - 1), __FILE__, __LINE__);
      soffset += fmax2;
      while(!endmsg)
      {
	lseek(ttarea->sqd.sqd, soffset, SEEK_SET);
...
//      if(!(mode&2176))
	...
	smsgcrc32 = crc32block(bufsqd.toname, (short)stolen, smsgcrc32);
	smsgcrc32 = crc32block(bufsqd.subj, (short)ssubjlen, smsgcrc32);
	if(curmsgid[0])
...
short sqhtobuf(long number)
{
  ...
  if(number >= (newarea->curindex + bufsqi) || number < newarea->curindex)
  {
    ...
    rread(newarea->sqd.sqi, nindex, (unsigned short)(bufsqi * 12), __FILE__,
	  __LINE__);
  }
