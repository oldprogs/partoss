...
/*  short finis=0; */
  ...
  if(arealist[0] == 0 && !(mode & 1092))
  {
	...
	if((alog =
	    (short)sopen(echologt, O_RDWR | O_BINARY, SH_DENYWR)) == -1)
	{
	  if((alog =
...
//       }
	...
	do
	{
	  ...
	  while(!endstring[2])
	  {
	    ...
	    {
	      for(carea = 0; carea < tlist->numlists; carea++)
		if(strnicmp(token, tlist->alist[carea].areaname, toklen) == 0
		    && strlen(tlist->alist[carea].areaname) == toklen)
		{
		  scanarea(tlist->alist[carea].areaoffs, type);
		  goto nfound2;
		}
  ...
  {
    if(globold < 1024)
      sprintf(logout, "Total compressing: %ldb -> %ldb, %d%%", globold,
	      globnew, (globnew * 100) / globold);
    else
      sprintf(logout, "Total compressing: %ldK -> %ldK, %d%%", globold >> 10,
	      globnew >> 10,
...
void scansqds(long aoffset)
{
  ...
  else
  {
	...
	if((strcmpi(newarea->areaname, persarea->areaname) == 0) && (mode & 3)
	    && !(mode & 1))
	{
	  frompers = 1;
	  arealock = 0;
	...
	{
	  scansqd();
	  if(newarea->passthr && !bcfg.passpurge && (newarea->sqd.sqd > 0))
	  {
  ...
  if(maxmsg)
  {
	...
	rread(newarea->sqd.sqi, nindex, (unsigned short)(bufsqi * 12),
      ...
      wwrite(newarea->sqd.sqd,
	     &(nindex[(unsigned)(i - newarea->curindex)].umsgid), 4, __FILE__,
	     __LINE__);
      delctrl(2);
    mnext5:
      ;
	...
	if(!notouch || compaddr(notouch->list.chain, &node))
	{
	  if(mmsgout() == 0)
	  {
	    ...
	    else
	    {
	      ...
	      rread(msg, &bufmsg, (unsigned short)(szmessage - szchar),
		    __FILE__, __LINE__);
	      bufmsg.flags |= 0x0008;
	      lseek(msg, 0, SEEK_SET);
	      wwrite(msg, &bufmsg, (unsigned short)(szmessage - szchar),
		     __FILE__, __LINE__);
...
/* remove unused variable LMA!!!        if((temp=strstr(tdn->where,"*"))!=NULL || (temp=strstr(tdn->where,"?"))!=NULL)*/
	if(strstr(tdn->where, "*") != NULL || strstr(tdn->where, "?") != NULL)
	{
//          if(strnicmp(newarea->areaname,tdn->where,temp-tdn->where)==0)
	...
	if(((buftemp->fromzone == tdn->from.zone)
	    ...
	    && ((plink->zone == tdn->to.zone) || (tdn->to.zone == 65535u))
	    && ((plink->net == tdn->to.net) || (tdn->to.net == 65535u))
	    && ((plink->node == tdn->to.node) || (tdn->to.node == 65535u))
	    && ((plink->point == tdn->to.point) || (tdn->to.point == 65535u)))
	  match = 1;
      }
      else
      {
	...
	{
	  if(((plink->zone == tdn->to.zone) || (tdn->to.zone == 65535u))
	      && ((plink->net == tdn->to.net) || (tdn->to.net == 65535u))
	      && ((plink->node == tdn->to.node) || (tdn->to.node == 65535u))
  ...
  stopit:
	...
	while(glink)
	{
	  ...
	  {
	    if((glink->zone == plink->zone) && (glink->net == plink->net)
	...
	while(ttaddr)
	{
	  ...
	  {
	    if((ttaddr->zone == plink->zone) && (ttaddr->net == plink->net)
...
//        memcpy(&(tpack->fromaddr),ttaddr,szmyaddr);
    ...
    if(tpack)
    {
      ...
      {
	if((tpack->handle =
	    (short)sopen(tpack->name, O_RDWR | O_BINARY, SH_DENYWR)) == -1)
	{
	  switch (errno)
	  {
	    case ENOENT:
		...
		if(errno == EMFILE)
		{
		  ...
		  tpack->outaddr.zone = tpack->outaddr.net =
		    tpack->outaddr.node = tpack->outaddr.point = 0;
		}
		else
