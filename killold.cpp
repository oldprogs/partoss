...
void killold(void)
{
  ...
  while(blink)
  {
    ...
    sprintf(logout, "%u:%u/%u.%u~%u:%u/%u.%u", link->zone, link->net,
        link->node, link->point, blink->address.zone,
        blink->address.net, blink->address.node,
        blink->address.point);
...
// Find closest matched address
    if(blink->address.point)
    {
      ...
      while(link)
      {
        ...
        && (link->node == blink->address.node) && (link->point == 0))
          goto lnkfound;
        link = link->next;
      }
    }
    else
    {
      ...
      {
        if((link->zone == blink->address.zone)
        && (link->net == blink->address.net) && (link->point == 0))
          goto lnkfound;
        link = link->next;
      }
      if(link == NULL)
      {
        ...
        if(link == NULL)
        {
          ...
          while(link)
          {
        if(link->zone == blink->address.zone && link->point == 0)
          goto lnkfound;
        link = link->next;
          }
        }
...
// Find closest matched address
    ...
    if((bcfg.killtempl[0] != DIRSEP[0]) && (bcfg.killtempl[2] != ':'))
    {
      mystrncpy(ttempl, (char *)homedir, DirSize);
      mystrncat(ttempl, (char *)bcfg.killtempl, DirSize, DirSize);
    }
    if((tmpl = (short)sopen(ttempl, O_RDONLY | O_BINARY, SH_DENYWR)) > 0)
    {
...
short killbox(struct link *blink)
{
  ...
  for(i = 0; i < 4; i++)
  {
    ...
    fname[i + 4] = duotrice[((blink->address.node) >> ((3 - i) << 2)) & 0xf];
    fpname[i] = '0';
    fpname[i + 4] =
    ...
    if(diff >= blink->days)
    {
      ...
      if(boxes != 1)
      {
        if(boxes < 4)
        {
          ...
          nomore = (short)_dos_findfirst(tmpfile, findattr, &tblk);
          while(nomore == 0)
          {
        if(!(tblk.attrib & 0x1f))
        {
          ...
          {
            temp = strstr(strupr(bufmsg.subj), strupr(badname));
            if(temp)
            {
            ...
            *temp = ' ';
            ...
            wwrite(msg, bufmsg.subj, 72, __FILE__, __LINE__);
        ...
        else
        {
          ...
          mystrncat(binkname, blink->address.point ? fpname : fname, 10,
            DirSize);
          mystrncat(binkname, ".?lo", 5, DirSize);
          i = (short)_dos_findfirst(binkname, findattr, &tblk);
          if(i == 0)
          {
        sprintf(logout, "Try to clean %s from %s", badname, binkname);
        logwrite(1, 10);
        ...
        if((j =
            (short)sopen(tbinkname, O_RDWR | O_BINARY | O_CREAT,
                 SH_DENYWR,
        ...
        while(!endinput[0])
        {
          ...
          do
          {
            ...
            if((temp =
            strstr(strupr(logout), strupr(badname))) == NULL)
              wwrite(j, string, maxstr2[0], __FILE__, __LINE__);
          }
          while(!endblock[0]);
        }
        close(i);
